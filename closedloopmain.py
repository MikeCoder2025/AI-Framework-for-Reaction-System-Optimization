...
def obs_FIM_ed(up,y,model,pe,pdpe, adjusted_determinants=None):
    """
    A function to calculate observed Fisher information matrix
    This corresponds to FIM computed based on measured 
    ...
    #print(adjusted_determinants)
    #print(np.shape(up)[0])
    for i in range(np.shape(up)[0]):
        if adjusted_determinants is not None:
            # Use the adjusted determinant instead of the FIM
            determinant = adjusted_determinants[i]
            # Incorporate the determinant into the Fisher Information calculation as needed
            # This will depend on how you want to adjust your FIM calculation
            obsFIM += determinant * FIM_fun(up[i], y, model, pe, pdpe)
        else:
            obsFIM += FIM_fun(up[i], y, model, pe, pdpe)
...
def initialisation1_mbdoemd_BF(y,m2,m3,pe2,pe3,p2,p3,pdtheta,n_dexp,n_u):
    ...
    for i in range(np.shape(y)[1]):
        mcov[i,i] = sigma[i]**2
    mc = 0.01
    control_bounds = [[319.0,320.0],[28.0,30.0],[0.020,0.025],[3.0,4.0],[1.6,1.6]]
    n_levels = 4
    ...
    for i in range(np.shape(ff_points)[0]):
        ff_actual[i,0] = ff_points[i,0] * ((320 - 319)/(n_levels-1)) + 319.0
        ff_actual[i,1] = ff_points[i,1] * ((30 - 28)/(n_levels-1)) + 28.0
        ff_actual[i,2] = ff_points[i,2] * ((0.025 - 0.020)/(n_levels-1)) + 0.0205
        ff_actual[i,3] = ff_points[i,3] * ((4 - 3)/(n_levels-1)) + 3.0
        ff_actual[i,4] = 1.6
...
def optimize_conversion(u,y,m3,pe3,p3,pdtheta,n_dexp,n_phi):
    # SEE: BuzziFerraris-Forzatti criterion
    """
    Objective function for MBDoE for model discrimination between two models
    ...
    exp_conversion = ((u[0:n_phi][2] - y_hat_m3[0][0]) / u[0:n_phi][2]) * 100
    return -1.0 * (exp_conversion)


import numpy as np

def optimize_conversion_ed(u, y, m3, pe3, p3, pdtheta, n_dexp, n_phi):
    # SEE: BuzziFerraris-Forzatti criterion
    """
    Objective function for MBDoE for model discrimination between two models
    Computes the conversion and a single uncertainty value.
    ...
    p3 : TYPE
        Prior information for model 3.
    pdtheta : TYPE
        Description of uncertainty in parameters.
    n_dexp : int
        Number of experiments.
    n_phi : int
        Number of variables in each experiment.

    Returns
    -------
    exp_conversion : float
        Conversion percentage.
    uncertainty_conversion : float
        Single uncertainty in the conversion.
    """

    sigma = [0.00043, 0.00202, 0.00051]  # Standard deviations for measurements
    mcov = np.identity(np.shape(y)[1])
    
    # Set up the measurement covariance matrix
    for i in range(np.shape(y)[1]):
        mcov[i, i] = sigma[i]**2
    
    mc = 0.01  # Model constant
    n_ymeas = np.shape(y)[1]
    y_hat_m3 = []
    
    # Simulate model m3 for each experiment
    for j in range(n_dexp):
        x0 = [u[j * n_phi:(j + 1) * n_phi][2], u[j * n_phi:(j + 1) * n_phi][2] * u[j * n_phi:(j + 1) * n_phi][3], 0.0, 0.0000000001]
        t = np.linspace(0.0, mc, 50)
        y_hat_m3 += [odeint(m3, x0, t, args=(np.hstack((u[j * n_phi:(j + 1) * n_phi], np.array([1.27]))), pe3, 
                         (Pinmodel(np.hstack((u[j * n_phi:(j + 1) * n_phi], np.array([1.27]))), pdtheta)[0] + 1.27) / 2))[-1][:n_ymeas]]
    
    # Sensitivity matrix and Fisher Information Matrix (FIM)
    exp_sen3 = np.zeros([np.shape(y)[1], np.shape(pe3)[0]])
    for j in range(n_dexp):
        exp_sen3 += sen_fun(np.hstack((u[j * n_phi:(j + 1) * n_phi], np.array([1.27]))), y, m3, pe3, pdtheta)
    
    exp_FIM3 = np.zeros([np.shape(pe3)[0], np.shape(pe3)[0]])
    exp_FIM3 += p3  # Adding prior information
    
    # Update the FIM for each experiment
    for j in range(n_dexp):
        exp_FIM3 += FIM_fun(np.hstack((u[j * n_phi:(j + 1) * n_phi], np.array([1.27]))), y, m3, pe3, pdtheta)
    ...
    # Calculate conversion based on model results
    exp_conversion = ((u[0:n_phi][2] - y_hat_m3[0][0]) / u[0:n_phi][2]) * 100

    # To get a single uncertainty, use the trace (sum of variances) or the average variance
    # Taking the square root gives the overall standard uncertainty
    uncertainty_conversion = np.sqrt(np.trace(exp_pcov3) / np.shape(exp_pcov3)[0])
    # print("this is the uncertainty in the conversion", uncertainty_conversion)
    # Return the conversion and the uncertainty
    return -1.0 * exp_conversion, uncertainty_conversion
...
def initialisation1_mbdoepp(y,model,pe,prior,pdtheta,n_dexp,n_u):
    ...
    """
    control_bounds = [[300.0,320.0],[25.0,30.0],[0.005,0.025],[2.0,4.0],[1.3,1.3]]
    # control_bounds = [[200.0,300.0],[15.0,20.0],[0.005,0.025],[1.50,3.50],[1.3,1.3]]
    n_levels = 16
    ff_points = fullfact([n_levels,n_levels,n_levels,n_levels])
    n_samples = np.shape(ff_points)[0]
    ff_actual = np.zeros([np.shape(ff_points)[0],n_u])
